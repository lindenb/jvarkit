...
public class BlastMapAnnotations
	extends AbstractBlastMapAnnotations
	{
	private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(BlastMapAnnotations.class);
	...
	static private class MyCommand extends AbstractBlastMapAnnotations.AbstractBlastMapAnnotationsCommand
			{    
		...
		protected Collection<Throwable> call(String inputName) throws Exception
			...
			try
				...
				if(IN==null)
					{
					return wrapException("UNDEFINED INPUT (IN)");
					}
				...
				if("GBSet".equals(domEntry.getDocumentElement().getNodeName()))
					...
					{
					return wrapException("unknown root element:"+domEntry.getDocumentElement().getNodeName());
					}
				Document blastDom;
				if(inputName!=null)
					{
					LOG.info("reading "+inputName);
					blastDom=docBuilder.parse(new File(inputName));
					}
				else
					{
