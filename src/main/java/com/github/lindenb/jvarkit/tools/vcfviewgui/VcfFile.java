...
public abstract class VcfFile implements NgsFile<VCFHeader,VariantContext>{
	...
	protected VcfFile()  throws IOException
		{
		}
	...
	public static VcfFile newInstance(final URL url) throws IOException {		
		...
		return new RemoteVcfFile(url,tbiFile);
		}
	...
	LocalVcf(final File f) throws IOException
		{
		this.file=f;
	...
	RemoteVcfFile(final URL url,final File indexFile) throws IOException
		{
		this.url = url;
	...
	@Override
		public VcfFile reOpen() throws IOException {
			final RemoteVcfFile bf= new RemoteVcfFile(this.url, this.indexFile);
