...
Usage: vcfensemblvep [options] Files
  Options:
    ...
    -x, --xslt, --template
      ...
      https://gist.github.com/lindenb/12dea8e701d18280e8cb0b65270064f6 
...
<xsl:text>
 ...
 <xsl:for-each 
      select="intergenic_consequences">
   <xsl:if 
      test="position()&gt;1">,</xsl:if>
   <xsl:apply-templates select="."/>
...
<xsl:text>
 ...
 <xsl:if 
      test="regulatory_feature_consequences">
 
      <xsl:text>VEPREST_REGULATORY_CSQ=</xsl:text>
 <xsl:for-each 
      select="regulatory_feature_consequences">
   <xsl:if 
      test="position()&gt;1">,</xsl:if>
   <xsl:apply-templates select="."/>
